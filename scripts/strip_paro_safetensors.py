#!/usr/bin/env python3
"""Create a stripped ParoQuant safetensors export.

ParoQuant HF-bridge exports currently contain both quantized tensors
(`.qweight/.qzeros/.scales/...`) and duplicate fp16 `.weight` fallback tensors
for the same modules.  These fallback tensors make the artifact BPW look heavier
than the deployable packed representation.

Modes:
  hipengine  Remove duplicate fp16 fallbacks that hipENGINE does not need today,
             but keep `mlp.shared_expert.*.weight` because current hipENGINE
             prepares shared-expert W8A16 tensors from those fp16 weights.
  packed     Remove every `.weight` whose module also has `.qweight`.  This is
             the smallest representation, but current hipENGINE needs a loader
             change (or preprepared shared-expert runtime tensors) before it can
             consume it.
...
def should_keep_duplicate_fallback(key: str, *, mode: str) -> bool:
    if mode == "packed":
        return False
    if mode == "hipengine":
        # Current hipENGINE builds shared-expert W8A16 runtime tensors from
        # these fp16 weights.  Removing them requires a loader/runtime change.
        return ".mlp.shared_expert." in key
    raise ValueError(f"unknown mode {mode!r}")


def plan_removal(keys: set[str], *, mode: str) -> set[str]:
    prefixes = quant_prefixes(keys)
    remove: set[str] = set()
    for key in keys:
        if is_duplicate_fallback(key, prefixes) and not should_keep_duplicate_fallback(key, mode=mode):
            remove.add(key)
    return remove
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=["hipengine", "packed"], default="hipengine")
    ...
    prefixes = quant_prefixes(set(sizes))
    remove = plan_removal(set(sizes), mode=args.mode)
    ...
    payload = {
        ...
        "kept_duplicate_shared_expert_fallbacks": sorted(k for k in kept if is_duplicate_fallback(k, prefixes)),
